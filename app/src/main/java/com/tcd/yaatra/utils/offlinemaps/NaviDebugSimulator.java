...
public class NaviDebugSimulator {
  ...
  public void setSimuRun(boolean run) {
    debug_simulator_run = run;
  }

  /**
   * The DEBUG_SIMULATOR will simulate first generated route on naviStart and trackingStart.
   **/
  public void startDebugSimulator(final OfflineMaps offlineMaps, InstructionList instructions, boolean fromTracking) {
    if (!DEBUG_SIMULATOR) {
      return;
    }
    ...
    runDelayed(offlineMaps, pLoc, lastLoc, 0);
  }

  private void runDelayed(final OfflineMaps offlineMaps, final Location pLoc, final Location lastLoc, final int index) {
